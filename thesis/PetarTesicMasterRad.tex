...
\apstr{
U savremenoj industriji razvoja softvera, efikasnost i optimizacija koda predstavljaju 
ključne aspekte u postizanju visokih performansi računarskih sistema. Ovaj master rad 
istražuje inovativan pristup prevođenju algoritma CRC (eng. \textit{Cyclic Redundancy 
Check}) korišćenjem kompilatorske infrastrukture LLVM. Algoritam CRC detektuje 
potencijalne promene u podacima nastalim usled transfera kroz različite medijume 
(žičane mreže, bežične mreže ili optičke kablove) i ima široku primenu u digitalnoj 
komunikaciji, gde se koristi za proveru integriteta podataka. Zbog svoje učestale 
primene važno je koristiti optimizovane verzije ovog algoritma.
 
Cilj ovog rada je unapređenje infrastrukture LLVM u kontekstu prevođenja algoritma 
CRC, i na taj način ostvarivanje boljih performansi programa koji koriste algoritam 
CRC i LLVM kao svoj kompilator. Osnovna ideja rešenja predstavljenog u radu zasniva se 
na detekciji neoptimizovane verzije algoritma CRC na nivou LLVM međureprezentacije i 
zameni funkcionalno ekvivalentnom optimizovanom verzijom. Unapređenje je vidljivo
na različitim procesorskim arhitekturama, specijalno i na procesorskoj arhitekturi 
RISC-V. Rezultati dobijeni testiranjem predstavljenog rešenja pokazuju značajno 
poboljšanje performansi algoritma CRC prevedenog LLVM kompilatorom.
\\
}
...
%Rezultati pokazuju za čak 35\% bolju vremensku efikasnost programa nad kojim je pokrenuta jedna od predloženih implementacija. 
...
\centering
 ...
 \centering
 %Nodes
 \node[squarednode]      (interpreter_node)  {\textbf{Interpretator}};
...
%\href{https://github.com/llvm/llvm-project}{GitHub} 
...
% Pošto kontrolni bitovi imaju fiksiranu dužinu, funkcija koja ih generiše se obično naziva i heš funkcija.
...
% Proširenja za manipulaciju bitovima
...
% Ideja prepoznavanja i optimizovanja algoritma podseća na neki od algoritama za pretragu teksta (tražimo jedan obrazac u tekstu odnosno LLVM međukodu i želimo da ga zamenimo drugim). Međutim, u okviru kompilatora LLVM tekst koji pretražujemo je u memoriji sačuvan kao 
...
% Mehanizam prepoznavanja dostupan u okviru kompilatora LLVM, pored toga što prepoznaje potpis neke instrukcije i redosled instrukcija on proverava i da li argumenti instrukcija dolaze iz odgovarajućih virtuelnih registara (inicijalizovanih tačno određenim instrukcijama). Takođe, korišćeni mehanizam nam daje garanciju da određene instrukcije pripadaju određenom osnovnom bloku što kod algoritama koji se bave pretragom teksta ne bi bio slučaj. Takvi algoritmi uglavnom sav sadržaj teksta smatraju ravnopravnim i nisu u stanju
...
%\footnote{Možda je to i razlog zašto implementacija opisana u članku koji je služio kao osnova za implementaciju nije integrisana u projekat GCC.}
